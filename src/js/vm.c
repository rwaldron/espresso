...
prog_char vm_string_0[] PROGMEM = "VM: out of memory";
prog_char vm_string_1[] PROGMEM = "js_vm_execute(): unknown constant type %d%s";
prog_char vm_string_2[] PROGMEM = "undefined function `%s' in apply";
prog_char vm_string_3[] PROGMEM = "undefiend function in apply";
prog_char vm_string_4[] PROGMEM = "call_method: unknown method";
prog_char vm_string_5[] PROGMEM = "illegal builtin object for call_method";
prog_char vm_string_6[] PROGMEM = "illegal object for call_method";
prog_char vm_string_7[] PROGMEM = "VM: error: %s%s";
prog_char vm_string_8[] PROGMEM = "VM: no valid error handler initialized%s";
...
JSVirtualMachine *
js_vm_create (unsigned int stack_size, unsigned int verbose,
...
js_vm_execute (JSVirtualMachine *vm, JSByteCode *bc)
{
  ...
  if (handler == NULL)
    {
      sprintf_P (vm->error, vm_string_0);
      return 0;
    }
  handler->next = vm->error_handler;
  vm->error_handler = handler;
	...
	if (bc->sects[sect].type == JS_BCST_CONSTANTS)
	  {
		...
		switch (c->type)
		  ...
		  default:
		  case JS_IPTR:
		    sprintf_P (buf, vm_string_1, c->type, JS_HOST_LINE_BREAK);
	     ...
	     unsigned int argc, JSNode *argv)
{
  ...
  if (setjmp (vm->error_handler->error_jmp))
	...
	{
	  if (func_name)
	    sprintf_P (vm->error, vm_string_2, func_name);
	  else
	    sprintf_P (vm->error, vm_string_3);
  ...
  if (setjmp (vm->error_handler->error_jmp))
	  ...
	  if (object->u.vbuiltin->info->method_proc)
	    {
		  ...
		  == JS_PROPERTY_UNKNOWN)
		{
		  sprintf_P (vm->error, vm_string_4);
		  result = 0;
		}
	    }
	  else
	    {
	      sprintf_P (vm->error, vm_string_5);
	      result = 0;
	    }
	}
	...
	to_builtin_please:
	  if ((*vm->prim[object->type]->method_proc) (vm,
	      ...
	      == JS_PROPERTY_UNKNOWN)
	    {
	      sprintf (vm->error, vm_string_4);
	      result = 0;
	    }
	}
      else
	{
	  sprintf (vm->error, vm_string_6);
	  result = 0;
	}
    }
  ...
  if (vm->num_globals >= vm->globals_alloc)
    {
      vm->globals = js_realloc (vm, vm->globals,
				(vm->globals_alloc + 16) * sizeof (JSNode));
      vm->globals_alloc += 16;
    }
...
js_vm_error (JSVirtualMachine *vm)
{
  ...
  if (vm->stacktrace_on_error)
    {
    sprintf_P (error, vm_string_7, vm->error, JS_HOST_LINE_BREAK);
  ...
  sprintf_P (error, vm_string_8, JS_HOST_LINE_BREAK);
  //js_iostream_write (vm->s_stderr, error, strlen (error));
  //js_iostream_flush (vm->s_stderr);
