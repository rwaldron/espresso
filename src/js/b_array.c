...
prog_char b_array_string_0[] PROGMEM =
        "Array.sort(): comparison function didn't return integer";
prog_char b_array_string_1[] PROGMEM =
        "Array.%s(): illegal amount of arguments";
prog_char b_array_string_2[] PROGMEM = "Array.%s(): illegal argument";
prog_char b_array_string_3[] PROGMEM = "Array.%s: immutable property";
...
sort_js_cmp_func (const void *aptr, const void *bptr, void *context)
{
  ...
  if (ctx->vm->exec_result.type != JS_INTEGER)
    {
      sprintf_P (ctx->vm->error, b_array_string_0);
      js_vm_error (ctx->vm);
    }
	...
	JSNode *args)
{
 ...
 argument_error:
  sprintf_P (vm->error, b_array_string_1, js_vm_symname (vm, method));
  js_vm_error (vm);

 argument_type_error:
  sprintf_P (vm->error, b_array_string_2, js_vm_symname (vm, method));
  js_vm_error (vm);
	  ...
	  void *instance_context, JSSymbol property, int set, JSNode *node)
{
 ...
 immutable:
  sprintf_P (vm->error, b_array_string_3, js_vm_symname (vm, property));
  js_vm_error (vm);
