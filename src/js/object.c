...
prog_char object_string_0[] PROGMEM = "load_property: illegal array index";
prog_char object_string_1[] PROGMEM =
        "store_array: array index can't be nagative";
prog_char object_string_2[] PROGMEM = "delete_array: illegal array index";
prog_char object_string_3[] PROGMEM =
			 ...
			 JSNode *value_return)
{
  if (sel->type == JS_INTEGER)
    ...
    {
      sprintf_P (vm->error, object_string_0);
      js_vm_error (vm);
    }
}
  ...
  if (sel->type == JS_INTEGER)
    {
      if (sel->u.vinteger < 0)
	{
	  sprintf_P (vm->error, object_string_1);
	  js_vm_error (vm);
	}
...
js_vm_object_delete_array (JSVirtualMachine *vm, JSObject *obj, JSNode *sel)
{
  if (sel->type == JS_INTEGER)
    ...
    {
      sprintf_P (vm->error, object_string_2);
      js_vm_error (vm);
    }
}
  ...
  if (b == NULL)
    {
      ...
      sprintf_P (buf, object_string_3, JS_HOST_LINE_BREAK);
