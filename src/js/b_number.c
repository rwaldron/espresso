...
prog_char b_number_string_0[] PROGMEM = "Number(): illegal amount of arguments";
prog_char b_number_string_1[] PROGMEM = "Number.%s(): illegal radix %d";
prog_char b_number_string_2[] PROGMEM =
        "Number.%s(): illegal amount of arguments";
prog_char b_number_string_3[] PROGMEM = "Number.%s(): illegal argument";
prog_char b_number_string_4[] PROGMEM = "Number.%s: immutable property";
prog_char b_number_string_5[] PROGMEM =
        "new Number(): illegal amount of arguments";
	       ...
	       JSNode *args)
{
  if (args->u.vinteger == 0)
    ...
    {
      sprintf_P (vm->error, b_number_string_0);
      js_vm_error (vm);
    }
}
	...
	JSNode *args)
{
  ...
  if (method == vm->syms.s_toString)
      ...
      if (n)
	{
	  ...
	  if (n->type == JS_INTEGER)
		...
		default:
		  sprintf_P (vm->error, b_number_string_1,
 ...
 argument_error:
  sprintf_P (vm->error, b_number_string_2, js_vm_symname (vm, method));
  js_vm_error (vm);

 argument_type_error:
  sprintf_P (vm->error, b_number_string_3, js_vm_symname (vm, method));
  js_vm_error (vm);
	  ...
	  void *instance_context, JSSymbol property, int set, JSNode *node)
{
 ...
 immutable:
  sprintf_P (vm->error, b_number_string_4, js_vm_symname (vm, property));
  js_vm_error (vm);
	  ...
	  JSNode *result_return)
{
  if (args->u.vinteger == 0)
    ...
    {
      sprintf_P (vm->error, b_number_string_5);
      js_vm_error (vm);
    }
}
